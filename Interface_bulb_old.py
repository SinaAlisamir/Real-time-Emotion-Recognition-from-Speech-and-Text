...
class Visualise():
    ...
    def fill_results_list(self):
        ...
        # print(self.backend.labels)
        self.emotion = self.backend.labels["pred"]
        print("emotion", self.emotion)
        loop = asyncio.get_event_loop()
        loop.run_until_complete(self.bulb_func())

    async def bulb_func(self):
        bulbs = await discovery.discover_lights(broadcast_space="192.168.1.255")
        print(f"Bulb IP address: {bulbs[0].ip}")
        self.myBulb = wizlight(bulbs[0].ip)
        neg = ['anger', 'despair', 'worry', 'irritation', 'sadness', 'fear']
        pos = ['fun', 'pride', 'interest', 'joy', 'pleasure', 'relief']
        color_1 = ['fun', 'joy', 'pleasure']
        color_2 = ['interest', 'pride', 'relief']
        color_3 = ['despair', 'worry', 'sadness']
        color_4 = ['anger', 'irritation', 'fear']
        brightness = 100
        if self.emotion in color_1:
            await self.myBulb.turn_on(PilotBuilder(rgb = (12, 157, 230), brightness = brightness))
        if self.emotion in color_2:
            await self.myBulb.turn_on(PilotBuilder(rgb = (220, 243, 12), brightness = brightness))
        if self.emotion in color_3:
            await self.myBulb.turn_on(PilotBuilder(rgb = (243, 182, 12), brightness = brightness))
        if self.emotion in color_4:
